...
import { Backdrop, Box, Button, Fade, Grid2, Modal, Paper, Tab, Tabs, Typography } from "@mui/material";
...
const FundingProjectDetailModal = ({ selectedFundingProjectId, openModal, setOpenModal, fetchFundingProjectList }) => {
    ...
    const fetchFundingProject = async () => {
        try {
            setIsLoading(true);
            const res = await fundingProjectApiInstance.get(`${selectedFundingProjectId}`)
            if (res.status == 200) {
    ...
    const handleChangeStatus = async (status) => {
        try {
            const message = status === 6 ? "Do you want to reject this project?" : status === 7 ? "Do you want to approve this project?" : "Do you want to block this project?";
            const actionText = status === 6 ? "Reject" : status === 7 ? "Approve" : "Block";
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const res = await fundingProjectApiInstance.put(`${selectedFundingProjectId}/status?status=${status}`, {
                            headers: {
    ...
    return (
        <>
            ...
            >
                <Fade in={openModal}>
                    <Box sx={style}>
                        {selectedFundingProject != null && JSON.stringify(selectedFundingProject) !== '{}' &&
                            <>
                                ...
                                >
                                        ...
                                        <Grid2 container columnSpacing={4} alignItems="center">
                                            <Grid2 size={5}>
                                                <div className="overflow-hidden flex justify-start items-center w-fit">
                                                    <img class=" h-[11rem] object-cover rounded-[0.625rem] w-[20rem]" alt="Funding project image" src={selectedFundingProject?.fundingFiles.find((e) => e.filetype == 2)?.url} />
                                                </div>
                                            </Grid2>
                                            <Grid2 size={7}>
                                                <Typography sx={{ mb: '0.5rem' }}>
                                                    <span
                                                        ...
                                                        style={{
                                                            backgroundColor: projectStatus[selectedFundingProject.status]?.color || "#9E9E9E",
                                                        }}
                                                    >
                                                        {projectStatus[selectedFundingProject.status]?.name || "Unknown"}
                                                    </span>
                                                </Typography>
                                                <Typography sx={{ fontSize: '1.25rem', fontWeight: '700', mb: '0.25rem', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '25rem' }}>
                                                    {selectedFundingProject.name}
                                                </Typography>
                                                <Typography sx={{ fontSize: '1rem', fontWeight: '500', mb: '1rem', color: 'var(--grey)' }}>
                                                    {new Date(selectedFundingProject.startDate).toLocaleString()} <ArrowRightAlt /> {new Date(selectedFundingProject.endDate).toLocaleString()}
                                                </Typography>
                                                {selectedFundingProject.status !== undefined &&
                                                    ![0, 1, 6, 7].includes(selectedFundingProject.status) && (
                                                        <button
                                                            onClick={() => {
                                                                window.open(`/funding-detail/${selectedFundingProject?.id}`, "_blank");
                                                            }}
                                    ...
                                    >
                                        ...
                                        >
                                            <Tab
                                                ...
                                                sx={{
                                                    textTransform: "none",
                                                    color: activeTab === 0 ? "var(--primary-green) !important" : "var(--grey)",
                                            ...
                                            <Tab
                                                ...
                                                sx={{
                                                    textTransform: "none",
                                                    color: activeTab === 1 ? "var(--primary-green) !important" : "var(--grey)",
                                            ...
                                            <Tab
                                                ...
                                                sx={{
                                                    textTransform: "none",
                                                    color: activeTab === 2 ? "var(--primary-green) !important" : "var(--grey)",
                                    ...
                                    {activeTab == 0 ? <FundingProjectOverview fundingProject={selectedFundingProject} /> : activeTab == 1 ? <FundingProjectPackages packages={selectedFundingProject.packages} /> : <FundingProjectDonation fundingProjectId={selectedFundingProjectId} />}
                                </Box>
                                <div className="mx-[0.5rem] pt-[1rem] justify-end gap-[1rem] shadow-[0_-4px_4px_-2px_rgba(0,0,0,0.1)] h-[3.5rem]" style={{ display: selectedFundingProject?.status == 1 ? 'flex' : 'none' }}>
                                    <Button
                                        variant="contained"
                                ...
                                <div className="mx-[0.5rem] pt-[1rem] justify-end gap-[1rem] shadow-[0_-4px_4px_-2px_rgba(0,0,0,0.1)] h-[3.5rem]" style={{ display: [2].includes(selectedFundingProject?.status) ? 'flex' : 'none' }}>
                                    <Button
                                        variant="contained"
                                        className="manage-user-detail-block-button"
