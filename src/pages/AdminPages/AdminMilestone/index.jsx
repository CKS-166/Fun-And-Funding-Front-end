          ...
          <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            <div className="w-full md:w-1/2">
              <span className="text-sm font-normal text-gray-500">
                Showing <span className="font-semibold text-gray-900">{data?._data.totalItems} </span>
            ...
            <table className="w-full text-sm text-left text-gray-500">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3">Funding Project</th>
              ...
              <tbody>
                {data?._data.items.map((pm, index) => (
                  <tr key={index} className="border-b">
                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{pm.milestoneName}</th>
                    <td className="px-4 py-3">{new Date(pm.createdDate).toLocaleString()}</td>
                    <td className="px-4 py-3">{new Date(pm.endDate).toLocaleString()}</td>
                    <td className="px-4 py-3">
                      {/* <img className="w-10 h-10 rounded" src={pm.fundingProject.fundingFiles.} alt="Default avatar" /> */}
                      {pm.fundingProject.name}
                    </td>
                    <td className="px-4 py-3">{pm.milestone.disbursementPercentage * pm.fundingProject.balance} đ</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 flex items-center justify-end">
                      <button onClick={() => handleOpenModal(pm)} className="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none" type="button">
