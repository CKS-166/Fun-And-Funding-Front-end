...
function Media() {
    ...
    const handleThumbnailChange = (e, thumbnailId) => {
        ...
        if (file) {
            setThumbnail(prevThumbnails => {
                const thumbnailIndex = prevThumbnails.findIndex(item => item.id === thumbnailId && item.newlyAdded === false);
                const updatedThumbnails = [...prevThumbnails];
                if (thumbnailIndex != -1) {
                    updatedThumbnails[thumbnailIndex].isDeleted = true;
                } else {
                    const filteredThumbnail = updatedThumbnails.filter(item => item.id !== thumbnailId);
    ...
    const handleVideoChange = (e, videoId) => {
        ...
        if (file) {
            setProjectVideo(prevVideos => {
                const videoIndex = prevVideos.findIndex(item => item.id === videoId && item.newlyAdded === false);
                const updatedVideos = [...prevVideos];
                if (videoIndex != -1) {
                    updatedVideos[videoIndex].isDeleted = true;
                } else {
                    const filteredVideo = updatedVideos.filter(item => item.id !== videoId);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setProjectImages(prevImages => {
                        const imageIndex = prevImages.findIndex(item => item.id === bonusImage.id && item.newlyAdded === false);
                        const updatedImages = [...prevImages];
                        if (imageIndex != -1) {
                            updatedImages[imageIndex].isDeleted = true;
                        } else {
                            const filteredVideo = updatedImages.filter(item => item.id !== bonusImage.id);
    ...
    const checkIfEdited = (updatedThumbnail, updatedVideos, updateBonusImages) => {
        if (updatedThumbnail !== project.existedFile.filter(file => file.filetype === 2) || updatedVideos !== project.existedFile.filter(file => file.filetype === 1) || updateBonusImages !== project.existedFile.filter(file => file.filetype === 4)) {
            setMediaEdited(true);
        } else {
    ...
    return (
        <div className='w-full pb-[3rem]'>
            ...
            <div className='basic-info-section'>
                ...
                {thumbnail && thumbnail.length > 0 && thumbnail.some(item => !item.isDeleted) ? (
                    thumbnail
                        .filter(item => !item.isDeleted)
                        .map((item) => (
                            <div key={item.id} className="relative w-[70%] h-[17.8rem] bg-[#000000] rounded-lg">
                                <img
                                    src={item.url}
                                    alt="Package preview"
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-start', width: '70%', mt: '1.5rem' }}>
                    <input
                        type="file"
                        accept="image/*"
                        ...
                        id="thumbnail-input"
                    />
                    <Button variant='contained' onClick={() => document.getElementById('thumbnail-input').click()} sx={{ backgroundColor: '#1BAA64', textTransform: 'none', fontWeight: '600' }} startIcon={<ChangeCircleIcon />}>
                        Change Thumbnail
                    </Button>
                </Box>
            </div>
            <div className='basic-info-section'>
                ...
                {projectVideo && projectVideo.length > 0 && projectVideo.some(item => !item.isDeleted) ? (
                    projectVideo
                        .filter(video => !video.isDeleted)
                        .map((video, index) => (
                            <div key={index} className="relative overflow-hidden rounded-[0.625rem] w-[70%]">
                                <ReactPlayer
                                    key={video.url}
                                    url={video.url}
                                    width="100%"
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-start', width: '70%', mt: '1.5rem' }}>
                    ...
                    />
                    <Button variant='contained' onClick={() => document.getElementById('video-file').click()} sx={{ backgroundColor: '#1BAA64', textTransform: 'none', fontWeight: '600' }} startIcon={<ChangeCircleIcon />}>
                        Change Video
                    </Button>
                </Box>
            </div>
            <div className='basic-info-section !mb-[2rem]'>
                ...
                {projectImages && projectImages.length > 0 && projectImages.some(item => !item.isDeleted) ? (
                    <ImageList sx={{ width: '70%', ml: '0 !important', maxHeight: '40rem', scrollbarWidth: 'thin' }} cols={3} rowHeight={160}>
                        {projectImages
                            .filter(item => !item.isDeleted)
                            .map((item, index) => (
                                <ImageListItem key={index} onClick={() => handleImageClick(index)} sx={{ bgcolor: '#000000' }}>
                                    <img
                                        src={item.url}
                                        alt={`Project image ${index + 1}`}
                                        loading="lazy"
                                        style={{ cursor: 'pointer', height: '5rem', objectFit: 'contain' }}
                                    />
                                </ImageListItem>
                            ))}
                    </ImageList>
                ...
                {isImageOpen && (
                    <Lightbox
                        mainSrc={projectImages.filter(item => !item.isDeleted)[photoIndex].url}
                        nextSrc={projectImages.filter(item => !item.isDeleted)[(photoIndex + 1) % projectImages.filter(item => !item.isDeleted).length].url}
                        prevSrc={projectImages.filter(item => !item.isDeleted)[(photoIndex + projectImages.filter(item => !item.isDeleted).length - 1) % projectImages.filter(item => !item.isDeleted).length].url}
                        onCloseRequest={() => setIsImageOpen(false)}
                        onMovePrevRequest={() =>
                            setPhotoIndex((photoIndex + projectImages.filter(item => !item.isDeleted).length - 1) % projectImages.filter(item => !item.isDeleted).length)
                        }
                        onMoveNextRequest={() =>
                            setPhotoIndex((photoIndex + 1) % projectImages.filter(item => !item.isDeleted).length)
                        }
                        toolbarButtons={[
                            <button
                                title="Delete"
                                ...
                                onClick={() => handleDeleteBonusImage(projectImages[photoIndex])}
                            >
                                🗑️
                            </button>
                        ]}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-start', width: '70%', mt: '1.5rem' }}>
                    ...
                    />
                    <Button variant='contained' onClick={() => document.getElementById('bonus-image-file').click()} sx={{ backgroundColor: '#1BAA64', textTransform: 'none', fontWeight: '600' }} startIcon={<AddCircleIcon />}>
                        Add More Image
                    </Button>
                </Box>
            </div>
